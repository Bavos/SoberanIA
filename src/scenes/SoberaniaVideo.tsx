...
const scenes = [
  {title: 'SoberanIA', subtitle: 'IA GENERATIVA BRASILEIRA', detail: 'O FUTURO DA GESTÃO PÚBLICA', accent: palette.neonGreen},
  {title: 'SOBERANIA DIGITAL', subtitle: 'Modelo Soberano 1', detail: 'Treinado em português e adaptado ao Brasil', accent: palette.electricBlue},
  {title: '6 FERRAMENTAS', subtitle: 'PARA TRANSFORMAR', detail: 'A GESTÃO PÚBLICA', accent: palette.vibrantYellow},
  {title: '1. GOV CHAT', subtitle: 'ATENDIMENTO INTELIGENTE', detail: 'AO CIDADÃO', accent: palette.neonGreen},
  {title: '2. BO FÁCIL', subtitle: 'REGISTRO POR VOZ', detail: 'OU TEXTO', accent: palette.electricBlue},
  {title: '3. SEDUC AI', subtitle: 'EDUCAÇÃO PERSONALIZADA', detail: 'COM IA', accent: '#A05DFF'},
  {title: '4. AGENTES SEI', subtitle: 'INTELIGÊNCIA PARA', detail: 'PROCESSOS ADMINISTRATIVOS', accent: '#38F6DB'},
  {title: '5. GERADOR DE TR', subtitle: 'TERMOS DE REFERÊNCIA', detail: 'EM SEGUNDOS', accent: '#FF9A00'},
  {title: '6. DEVKIT & APIs', subtitle: 'INTEGRE, CRIE', detail: 'TRANSFORME', accent: palette.neonGreen},
  {title: 'SoberanIA', subtitle: 'O FUTURO DA IA PÚBLICA', detail: 'É BRASILEIRO', accent: palette.vibrantYellow},
];
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{position: 'absolute', top: 90, left: 70, right: 70, textAlign: 'center'}}>
        <div style={{fontSize: 28, color: accent, letterSpacing: 3, fontWeight: 700}}>{`${index * 3}s - ${index * 3 + 3}s`}</div>
        <h1 style={{fontSize: 118, lineHeight: 0.95, margin: '20px 0 0', color: palette.white, textTransform: 'uppercase', textShadow: `0 0 42px ${accent}`}}>{title}</h1>
        <h2 style={{fontSize: 58, margin: '16px 0 0', color: accent, lineHeight: 1}}>{subtitle}</h2>
      </div>
      ...
      <div style={{position: 'absolute', left: 90, right: 90, top: 540, height: 860, borderRadius: 44, background: 'linear-gradient(165deg, #061321DD, #070A12EE)', border: `2px solid ${accent}AA`, boxShadow: `0 0 80px ${accent}99, inset 0 0 45px ${accent}55`}}>
        ...
        <div style={{position: 'absolute', left: 50, right: 50, top: 70, fontSize: 64, color: palette.white, fontWeight: 800, textAlign: 'center'}}>{detail}</div>
      ...
      <div style={{position: 'absolute', left: 30, right: 30, bottom: 42, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16}}>
        {['TECNOLOGIA', 'AGILIDADE', 'IMPACTO'].map((item) => (
          <div key={item} style={{padding: '20px 14px', borderRadius: 14, border: `1px solid ${accent}AA`, textAlign: 'center', color: palette.white, fontSize: 32, fontWeight: 700, background: '#060D19CC'}}>{item}</div>
        ))}
      </div>
